...
RedisClient.prototype.get_link_in_seeds = async function (index, options) {
    ...
    while (true) {
        ...
        if (link.crawled == '2') {
            // check if the index page is expired
            var obj = await this.get_link_index_status(index);
            if (obj) {
                // the index page is not yet expired no need to crawl again
                // unless the force option is set
                if (options && options.refresh_index) {
                    link.crawled = '-1';
                } else { console.error("index page hasn't expried, #" + index + ": " + link.url); }
            }
            else {
                // the index page is expired, need to crawl again
                link.crawled = '-1';
            }
            ...
            if (link.crawled == '-1') {
                ...
                await this.update_link_crawling_status(index, -1);
                console.log("updating index page: " + index + ", link status " + link.key + ", " + link.url + " to -1");
            }
        }
 ...
 * 
 * @param {*} index 
 * @param {*} status -1 - not crawled, 0 - in progress, 1 - crawled, 2 - crawled (index page)
 * @returns
 * @description update the crawling status of the link
 */
...
RedisClient.prototype.update_link_index_status = async function (index) {
    index = index || 0;
    let obj = {};
    // set the index page expiration time to at least 1 day
    obj.expire = Math.floor(Date.now() / 1000) + 86400;
...
RedisClient.prototype.get_link_index_status = async function (index) {
    index = index || 0;
    let obj = await this.promisify(
        this.version >= 4 ? this.client.hGet : this.client.hget,
        this.link_key_prefix + index,
        'index'
    );
    if (obj) {
        try {
            obj = JSON.parse(obj);
            if (obj.expire && obj.expire > Math.floor(Date.now() / 1000)) {
                return obj;
            }
            else {
                // remove the index page status if it has expired
                await this.promisify(
                    this.version >= 4 ? this.client.hDel : this.client.hdel,
                    this.link_key_prefix + index,
                    'index'
                );
                return null;
            ...
            console.error("Error parsing index status:", err);
            return null;
        }
    }
    return null;
