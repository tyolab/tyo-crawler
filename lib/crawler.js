...
function Crawler(opts) {
    ...
    // For keys 
    this.redis_client = new RedisClient(opts.redis);
        ...
        (async () => {
            try {
                ...
                if (!self.with_browser || (file && file.binary)) {
                    ...
                    try {
                        await self.page.goto(url, { waitUntil: 'networkidle2', timeout: wait_time || self.opts.browser.waitTimeout });
                        ...
                        if (options.with_cookies && cookies && Array.isArray(cookies)) {
                            for (const cookie of cookies) {
                                await self.page.setCookie(cookie);
                            }
                            const pageCookies = await self.page.cookies();
                            self.redis_client.set_cookies(pageCookies);
                        }
