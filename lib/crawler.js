...
// puppeteer-extra + the stealth plugin: a drop-in for puppeteer (same
// .launch()/.newPage() API) that patches the automation fingerprints anti-bot
// services flag — navigator.webdriver, headless/chrome.runtime hints, the
// permissions/plugins/WebGL-vendor tells, etc. Reduces our bot signature.
...
function Crawler(opts) {
    ...
    this.opts.browser.args = normalize_array_option(this.opts.browser.args);
    this.opts.browser.ignoreDefaultArgs = normalize_array_option(this.opts.browser.ignoreDefaultArgs);
...
function normalize_array_option(value) {
    ...
    if (this.with_browser) {
        if (!this.browser) {
            ...
            let browserOptions = {
                ...
                // dumpio: true,
                slowMo: this.opts.browser.slowMo || 0,
...
Crawler.prototype.crawl = async function (link, options, errorCallback) {
    ...
    async function save_cookies() {
        ...
        if (pageCookies && Array.isArray(pageCookies) && pageCookies.length > 0) {
            console.log("Saving cookies for: " + link.parsedUrl.host + " (" + pageCookies.length + " cookies)");
            await self.redis_client.set_cookies(pageCookies, link.parsedUrl.host);
            if (options.cookies_file) {
                const cookiesFile = path.resolve(options.cookies_file);
                fs.writeFileSync(cookiesFile, JSON.stringify(pageCookies, null, 2));
        ...
        (async () => {
            try {
                ...
                if (!self.with_browser || (file && file.binary)) {
                    ...
                    try {
                        await self.page.goto(url, { waitUntil: self.opts.browser.waitUntil || 'networkidle2', timeout: browser_wait_time || self.opts.browser.waitTimeout || 30000 });
                        ...
                        let has_page_been_processed = false;
                        ...
                        const apply_control = async () => {
                            if (!options.control || typeof options.control.checkpoint !== 'function') return {};
                            return await options.control.checkpoint();
                        };
                                    ...
                                    if (typeof action.time === 'number') {
                                        ...
                                        if (action.text) {
                                            ...
                                            await self.page.waitForSelector(action.selector, {
                                                timeout: action.timeout || action.timeout_ms || self.opts.browser.waitTimeout || 30000
                                            });
                                    }
                                }
                        ...
                        if (options.actions) {
                            let action_index = 0;
                            while (Array.isArray(options.actions) && action_index < options.actions.length) {
                                const control_result = await apply_control();
                                if (control_result.stop) break;
                                if (Array.isArray(control_result.actions)) {
                                    options.actions = control_result.actions;
                                    action_index = 0;
                                    continue;
                                }

                                const job = options.actions[action_index++];
                                if (job.if) {
                                    ...
                                    while (true) {
                                        let result = null;
                                        let should_reload_actions = false;
                                        for (const action of job.repeat) {
                                            ...
                                            if (action.action === 'process') {
                                                ...
                                                if (typeof result === 'object' && result.authRequired === true) {
                                                    console.warn("Authentication required, stopping repeat actions.");
                                                    failed_count = failed_limit;
                                                    break;
                                                }
                                                else if (typeof result === 'object' && result.failed === true) {
                                                    failed_count++;
                                                ...
                                                if (action.action === 'scroll') {
                                                    // action.value is a per-iteration delta; pass a copy so the
                                                    // parsed action object keeps its delta for the next iteration
                                                    scroll_height += action.value || 100;
                                                    result = await perform_action({ ...action, value: scroll_height });
                                                }
                                                else {
                                        ...
                                        if (should_reload_actions) break;
                        ...
                        if (!options.viewonly) {
                            if (!has_page_been_evaluated) {
                                ...
                                await process_page(result, callback, options, resolve);
                             }
                            else if (!has_page_been_processed && evaluated_result) {
                                console.log("Page has been evaluated, processing evaluated result...");
                                await process_page(evaluated_result, callback, options, resolve);
                            }
                            else {
                                console.log("Page has already been evaluated and processed, skipping final fallback...");
