...
function Crawler(opts) {
    ...
    this.index_pattern = opts.index_pattern || [];
}
...
Crawler.prototype.is_index_page = function (link) {
    ...
    async.eachSeries($(link_selectors), (elem, done) => {
        ...
        if (crawled_url) {
            if (crawled_url.match(/\\{2}/g))
                file = self.create_dest_file(crawled_url, { ext: '.html', type: type });
        }
        else {
            done();
            return;
...
Crawler.prototype.process_result = async function (result, options, resolve) {
    ...
    const link = new URL(result.href);
            ...
            try {
                ...
                if (!self.with_browser || (file && file.binary)) {
                    ...
                    try {
                        ...
                        if (!options.viewonly && !has_page_been_evaluated) {
