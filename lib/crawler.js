                        ...
                        const perform_action = async (action) => {
                            ...
                            try {
                                ...
                                if (action.action === 'click') {
                                    ...
                                    return await self.page.evaluate(evaluate_page);
                                }
                                    ...
                                    while (true) {
                                        ...
                                        for (const action of job.repeat) {
                                            if (action.action === 'process') {
                                                if (callback) {
                                                    result = await callback(evaluated_result, options);
                                                }
                                                else {
                                                    result = await self.process_result(evaluated_result, options);
                                                }
                                            }
                                            else {
                                                if (action.action === 'scroll') {
                                                    scroll_height += action.value || 1000;
